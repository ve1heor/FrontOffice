        ...
        db.run(query, (err, row) => {
        	if (err) {
        		try {
				  ...
				  // обработка ошибки
				  	console.log(tableName + " не создана!");
					return;
				}
				console.log(tableName + " успешно создана!");
		  	}
        })
...
function findDiscountCardByNumber(barcode){
	...
	if(flagDicountCard){
		if (ArrayProducts.length>0){
			db.get('SELECT id,barcode,percent FROM discount_card WHERE barcode = "' + barcode + '"', 
			  	(err, row) => {
				    if (err) {
				      console.error(err.message);
				    }
				    row
				    ? setDiscountCard(row.barcode, row.percent)
				    : alert('Не найдена дисконтная карта со штрихкодом ' + barcode);
...
function updateUser(id,login, password){
	db.run('update paymaster SET name ="'+
		login+'", password = "'+
		password+'"'+
		'WHERE id = "'+id+'"',
		(err, row) => {
...
function registerUser(login, password){
	db.run('INSERT INTO paymaster(name,password,isAdmin) VALUES("'+login+'","'+password+'","false")',
		(err, row) => {
		    if(err){
...
function executeOperation(oper_name, params) {
	db.serialize(() => {
		switch (oper_name) {
			case "getUsers":
				db.all('SELECT * FROM paymaster WHERE isAdmin = "false"', 
				  	(err, rows) => {
					    if (err) {
					      console.error(err.message);
					    }
			...
			case "getUserByLP":
				...
				db.get('SELECT name FROM paymaster WHERE name = "'+login+'" and password = "' + password + '"', 
				  	(err, row) => {
					    if (err) {
					      console.error(err.message);
					    }
			...
			case "saveUser":
				...
				db.get('SELECT id FROM paymaster'+
					' WHERE name = "'+login+'" and isAdmin = "false"', 
				  	(err, row) => {
					    if (err) {
					      console.error(err.message);
					    }
					    row
					    ? updateUser(row.id,login, password)
			...
			case "deleteUser":
				var login = params[0];
				db.run('DELETE FROM paymaster WHERE name="'+login+'"',
					(err, row) => {
					    if(err){
			...
			case "addProductsInDB":
				...
				$.each(params,function(index, row){
					db.all('SELECT id FROM product WHERE barcode = "'+row.barcode+'"', 
					  	(err, rows) => {
					  		if(rows.length>0){
					  			db.run("UPDATE product SET name = '"+row.name+
					  				"', article = '"+row.article+
					  				"', price = '"+row.price+
					  			...
					  			db.run('INSERT INTO product(name,article,barcode,price) VALUES("'+
					  				row.name+'","'+
					  				row.article+'","'+
					  				row.barcode+'","'+
					  				row.price+'")',
			...
			case "getOptions":
				options = [];
				db.all('SELECT name,value FROM option', 
				  	(err, rows) => {
					    if (err) {
					      console.error(err.message);
					    }
						...
						if(options.length == 0){
							...
							for(var i=0; i<options.length;i++){
								db.run('INSERT INTO option(name,value) VALUES("'+options[i].name+'","'+options[i].value+'")',
									(err, rows) => {
									    if (err) {
			...
			case "setOptions":
				for(var i=0; i<options.length;i++){
					db.run('UPDATE option SET value = "'+options[i].value+'" WHERE name = "'+options[i].name + '"',
						(err, rows) => {
						    if (err) {
						      return console.log(err.message);
						    }
